...
<section class="first-block wow fadeInUp clearfix" data-wow-delay="0.5s">
    ...
    ?>
            ...
            <?php      
                ...
                if( $block1_query->have_posts() ) {
                    while( $block1_query->have_posts() ) {
                        ...
                        $b1_image_path = wp_get_attachment_image_src( $b1_image_id, 'accesspress-mag-block-big-thumb', true );
            ...
            ?>
        <div class="single_post top-post non-zoomin clearfix">
            <?php if(has_post_thumbnail()) { ?>   
                <div class="post-image"><a href="<?php the_permalink();?>" title="<?php the_title(); ?>"><img src="<?php echo esc_url( $b1_image_path[0] );?>" alt="<?php echo esc_attr( $b1_image_alt );?>" /></a>
...
<section class="second-block clearfix wow fadeInLeft" data-wow-delay="0.5s">
    ...
    ?>
            <div class="carousel-block-wrapper">
                ...
                <div id="cat-block-carousel" class="block-post-wrapper owl-carousel clearfix">
            <?php                         
                $block2_args = array(
                                    'post_type' => 'post',
                ...
                if( $block2_query->have_posts() ) {
                    while( $block2_query->have_posts() ) {
                        ...
                        $b2_image_path = wp_get_attachment_image_src( $b2_image_id, 'accesspress-mag-block-big-thumb', true );
            ...
            ?>
                <div class="single_post item first-post non-zoomin clearfix">
                    <?php if( has_post_thumbnail() ) { ?>   
                        <div class="post-image"><a href="<?php the_permalink();?>" title="<?php the_title(); ?>"><img src="<?php echo esc_url( $b2_image_path[0] );?>" alt="<?php echo esc_attr( $b2_image_alt );?>" /></a>
                            <a class="big-image-overlay" href="<?php the_permalink();?>" title="<?php the_title(); ?>"><i class="fa <?php echo esc_attr( $apmag_overlay_icon );?>"></i></a>
                            <?php if( $show_icon == 'on' ){?><span class="format_icon"><i class="fa <?php echo esc_attr( $post_format_icon );?>"></i></span><?php } ?>    
                        </div><!-- .post-image-->
                    <?php } else {
        ...
        ?>
                </div><!-- #cat-block-carousel -->
            </div><!-- .carousel-block-wrapper -->
    <?php
        }
        wp_reset_query();
    ?>
            ...
            <?php                         
                ...
                if( $block3_query->have_posts() ) {
                    while( $block3_query->have_posts() ) {
                        ...
                        if( $b3_counter == 1 ){ echo '<div class="leftposts-wrapper">'; } 
                        if( $b3_counter > 1 && $b3_counter == 2 ){ echo '<div class="rightposts-wrapper">'; }
            ?>
                <div class="single_post clearfix <?php if( $b3_counter == 1 ){ echo 'first-post non-zoomin'; }?>">
                    <?php if( has_post_thumbnail() ) { ?>   
                        <div class="post-image"><a href="<?php the_permalink();?>" title="<?php the_title(); ?>"><img src="<?php if( $b3_counter <=1 ){echo esc_url( $b3_big_image_path[0] );}else{ echo esc_url( $b3_small_image_path[0] );}?>" alt="<?php echo esc_attr($b3_image_alt);?>" /></a>
                            <?php if( $b3_counter == 1 ) { ?> <a class="big-image-overlay" href="<?php the_permalink();?>" title="<?php the_title(); ?>"><i class="fa <?php echo esc_attr( $apmag_overlay_icon );?>"></i></a><?php } ?>
                            <?php if( $show_icon == 'on' ){?><span class="format_icon <?php if( $b3_counter > 1 ){ echo 'small'; }?>"><i class="fa <?php echo esc_attr( $post_format_icon );?>"></i></span><?php } ?>    
                        </div>
                    <?php } else {
                            if( $fallback_image_option == 1 && !empty( $fallback_image ) ) {
                    ?>
                    ...
                    ?>
                        <div class="post-desc-wrapper">
                            ...
                            <?php } ?>                                
                                <div class="block-poston">
                                    <?php 
                                        if( $b3_counter == 1 ) { accesspress_mag_home_author(); }
                                        do_action( 'accesspress_mag_home_posted_on' );
                                    ?>
                                </div><!-- .block-poston -->
                                <div class="ratings-wrapper"><?php do_action( 'accesspress_mag_post_review' );?></div>
                        </div><!-- .post-desc-wrapper -->
                        <?php if( $b3_counter == 1 ) { ?><div class="post-content"><?php the_excerpt(); ?></div><?php } ?>
                </div><!-- .single_post -->
            <?php 
                if( $b3_counter == 1 ){ echo '</div>'; } 
                if( $b3_counter > 1 && $b3_counter == $total_posts_block3 ){ echo '</div>'; }
                    }
                }
            ?>
                </div><!-- .block-post-wrapper -->
    ...
    <?php 
     ...
     ?>
            ...
            <?php      
                ...
                $b4_total_posts = $block4_query->post_count;
                $b4_counter = 0;
                if( $block4_query->have_posts() ) {
                    while( $block4_query->have_posts() ) {
                        ...
                        if( $b4_counter == 1 ) {
                            $b4_image_size = 'accesspress-mag-singlepost-default';
                        } else {
                            $b4_image_size = 'accesspress-mag-singlepost-style1';
                        }
                        $b4_image_path = wp_get_attachment_image_src( $b4_image_id, $b4_image_size, true );
                        ...
                        if( $b4_counter == 2 ) { echo '<div class="posts-list-wrapper">'; }
            ?>
                    <div class="single_post <?php if( $b4_counter == 1 ) { echo 'top-post non-zoomin'; } ?> clearfix">
                        <?php if( has_post_thumbnail() ) { ?>   
                            <div class="post-image"><a href="<?php the_permalink();?>" title="<?php the_title(); ?>"><img src="<?php echo esc_url( $b4_image_path[0] );?>" alt="<?php echo esc_attr( $b4_image_alt );?>" /></a>
                                <?php if( $b4_counter == 1 ) { ?><a class="big-image-overlay" href="<?php the_permalink();?>" title="<?php the_title(); ?>"><i class="fa <?php echo esc_attr( $apmag_overlay_icon );?>"></i></a><?php } ?>
                                <?php if( $show_icon == 'on' ){ ?><span class="format_icon"><i class="fa <?php echo esc_attr( $post_format_icon );?>"></i></span><?php } ?>
                            </div>
                        <?php } else {
                                if( $fallback_image_option == 1 && !empty( $fallback_image ) ) {
            ...
            <?php
                    if( $b4_counter == $b4_total_posts ) { echo '</div>'; }
                    }
                }
            ?>
            </div><!-- .block-post-wrapper -->
