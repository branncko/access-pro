...
if ( ! function_exists( 'accesspress_mag_paging_nav' ) ) :
/**
...
function accesspress_mag_paging_nav() {
	...
	$trans_older = of_get_option( 'trans_older_posts' );
	if( empty( $trans_older ) ) {
		$trans_older = __( 'Older posts', 'accesspress-mag' );
	}
	$trans_newer = of_get_option( 'trans_newer_posts' );
	if( empty( $trans_newer ) ) {
		$trans_newer =  __( 'Newer posts', 'accesspress-mag' );
	}
	?>
	<nav class="navigation posts-navigation clearfix" role="navigation">
		...
		<div class="nav-links">
			...
			<div class="nav-previous"><i class="fa fa-angle-left"></i><?php next_posts_link( $trans_older ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( $trans_newer ); ?><i class="fa fa-angle-right"></i></div>
			<?php endif; ?>
...
function accesspress_mag_posted_on() {
    ...
    $trans_by = of_get_option('trans_sep_by');
    if( empty( $trans_by ) ) {
    	$trans_by = __( 'by', 'accesspress-mag' );
    }
    ...
    if($show_author==1){
        $byline = sprintf(
    		_x( $trans_by.' %s', 'post author', 'accesspress-mag' ),
