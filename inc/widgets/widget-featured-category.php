...
class Apmag_Featured_Category extends WP_Widget {
    ...
    public function widget($args, $instance) {
        ...
        $trans_view_all = of_get_option( 'trans_view_all_posts', __( 'View all posts', 'accesspress-mag' ) );
        if( empty( $trans_view_all ) ) {
            $trans_view_all = __( 'View all posts', 'accesspress-mag' );
        }
        echo $before_widget; 
    ?>
                ...
                ?>
                    <div class="single_post clearfix <?php if( $e_counter == 1 ){ echo 'first-post non-zoomin'; } ?>">                        
                        ...
                        ?>
                            ...
                            <?php if( $e_counter > 1 ){ echo '</div>'; } if( $e_counter == 1 ):?><?php endif ;?>
                    </div><!-- .single_post -->
                <?php
                        }
                        if( !empty( $featured_category_view_all_show ) && $featured_category_view_all_show == 1 ) {
                            echo '<a href="'. esc_url( $cat_link ) .'" class="cat-link view-all-button">'. esc_html( $trans_view_all ) .'</a>';
                        }
                    }
                    }
                ?>    
