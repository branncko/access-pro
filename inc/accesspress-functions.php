 ...
 *  GLOBAL VARIABLES []
 */
$apmag_slider_option = of_get_option( 'slider_layout', 'slider-default' );
...
if( $slide_count == 0 ){
    ...
    $posts_perpage_value = 4;
} elseif( $apmag_slider_option == 'slider_highlight' ){
    $posts_perpage_value = $slide_count;
}else {
...
$posts_order_vlaue = apply_filters( 'accesspress_mag_slider_order', $posts_order_vlaue );
$slider_args = array(
            'post_type'=>'post',
            'post_status'=>'publish',
 ...
 */
 function apmag_admin_scripts(){
	wp_enqueue_script( 'apmag-custom-admin', OPTIONS_FRAMEWORK_DIRECTORY . 'js/custom-admin.js', array( 'jquery') );
 }
...
function accesspress_mag_slider_cb(){
        global $slider_args, $category_box_option, $slide_info;
        ...
        if( $slider_query->have_posts() )
        {
            ...
            ?>                        
                    <div class="slider">
                        <div class="apmag-slider-bigthumb wow fadeInLeft">                            
                            <div class="big_slide">
                                ...
                                <?php if( $slide_info == 1 ) { ?>
                                <a href="<?php echo the_permalink();?>" title="<?php the_title(); ?>">
                                    <div class="mag-slider-caption"><h2 class="slide-title"><?php the_title();?></h2></div>
                                </a>
                                <?php } ?>
                            </div><!-- .big_slide -->                            
                        </div><!-- .apmag-slider-bigthumb -->
            ...
            <?php } else { if( $slide_counter%4 == 2 ){ echo '<div class="small-slider-wrapper wow fadeInRight">'; } ?>                
                       ...
                       <a href="<?php echo the_permalink();?>" title="<?php the_title(); ?>">
                        <div class="small_slide">
                            ...
                            <div class="mag-small-slider-caption"><?php if( $slide_info == 1 ) { ?><h3 class="slide-title"><?php the_title();?></h3><?php } ?></div>                            
                        </div><!-- .small_slide -->
                       </a>
                       <?php 
            ...
            ?>
                    <div class="slider">
                        <div class="apmag-slider-single">
                            ...
                            </div><!-- .big-single-slide -->
                            </a>
                        </div><!-- .apmag-slider-single -->
                    </div><!-- .slider -->
            <?php
                }                
            }                
...
/*------------------------------------------------------------------------------------------------*/
/**
 * Homepage Slider with highlighted section
 */
if( !function_exists( 'accesspress_mag_slider_highlighted_cb' ) ):
    function accesspress_mag_slider_highlighted_cb() {
        global $slider_args, $category_box_option, $slide_info;
?>
        <div class="slider-highlight-section">
            <div class="apmag-slider-area">
            <?php
                $htd_slider_query = new WP_Query( $slider_args );
                if( $htd_slider_query->have_posts() ) {
                    echo '<div id="homeslider">';
                    while ( $htd_slider_query->have_posts() ) {
                        $htd_slider_query->the_post();
                        $post_image_id = get_post_thumbnail_id();
                        $post_single_image_path = wp_get_attachment_image_src( $post_image_id, 'accesspress-mag-slider-big-thumb', true );
                        $post_image_alt = get_post_meta( $post_image_id, '_wp_attachment_image_alt', true );
            ?>
                    <div class="htdslider">
                        <div class="apmag-slider-single">
                            ...
                            <div class="big-single-slide wow fadeInLeft">
                                ...
                                <?php if( $slide_info == 1 ) { ?> <a href="<?php echo the_permalink();?>"><div class="mag-slider-caption"><h2 class="slide-title"><?php the_title();?></h2></div></a><?php } ?>
                            </div><!-- .big-single-slide -->
                            </a>
                        </div><!-- .apmag-slider-single -->
                    </div><!-- .slider -->
...
add_action( 'accesspress_mag_slider_highlighted', 'accesspress_mag_slider_highlighted_cb', 10 );
/*---------------------------------------------------------------------------------------------------------*/
/**
...
if( !function_exists( 'accesspress_mag_non_slider_cb' ) ):
    function accesspress_mag_non_slider_cb() {
        global $slider_category, $slider_post_option, $category_box_option, $slide_info, $posts_order_vlaue;
        $non_slider_post_option = $slider_post_option;
        $non_slider_category = $slider_category;
                        ...
                        <div class="grid-post-thumb">
                            ...
                            <div class="grid-post-info-wrapper">
                                <div class="grid-post-info">
                                    <?php if( $category_box_option == '1' ) { ?><div class="grid-post-categories"><?php category_details( get_the_ID() ); ?></div> <?php } ?>
                            ...
                            </div><!-- .grid-post-info-wrapper -->
                            <?php }//endif slider_info ?>    
                        </div><!-- .grid-post-thumb -->                        
                    </div><!-- .grid-big-post -->
            <?php
                } else {
...
?>
        <div class="popular-block-wrapper">
                        ...
                        <div class="single_post clearfix <?php if( $p_counter == 1 ) { echo 'first-post'; } ?> non-zoomin">
                            <?php if( has_post_thumbnail() ) { ?>   
                                <div class="post-image">
                                    <a href="<?php the_permalink();?>" title="<?php the_title(); ?>"><img src="<?php echo esc_url( $p_image_path[0] );?>" alt="<?php echo esc_attr( $p_image_alt );?>" /></a>
            ...
            <?php
                        if( $p_counter > 1 && $p_counter == $p_total_posts ) { echo '</div>'; }
                    }
                }
            ?>
        ...
        ?>
            <div class="post-review-wrapper">
                ...
                <div class="summary-wrapper clearfix">
                    ...
                    </div><!-- .total-reivew-wrapper -->
                </div><!--. summary-wrapper -->
            </div><!-- .post-review-wrapper -->
        <?php
            } // endif $star_rating
        } //endif $post_review_type
       ...
       ?>
            <div class="post-review-wrapper">
                ...
                <div class="summary-wrapper clearfix">
                    ...
                    <div class="total-reivew-wrapper"><span class="total-value"><?php echo esc_attr ( $total_review ) ;?></span></div>
                </div><!-- .summary-wrapper -->
            </div><!-- .post-review-wrapper -->
       <?php    
            }// endif $points_rating
...
function accesspress_mag_sidebar_layout_class($classes){
    ...
    if ( is_woocommerce_activated() ) {
        if( is_shop() || is_product_category() ) {
            add_filter('loop_shop_columns', 'apmag_loop_columns');
            function apmag_loop_columns() {
                return 3;
            }
            $classes[] = 'columns-'.apmag_loop_columns();
        }
    }
	return $classes;
}
...
if( !function_exists( 'accesspress_mag_hex_to_rgba' ) ) :
    function accesspress_mag_hex_to_rgba( $color, $opacity = false ) {
    $default = 'rgb(0,0,0)'; 
...
/*--------------------------------------------------------------------------------------------------------*/
/**
 * Homepage author option
 */
if( !function_exists( 'accesspress_mag_home_author' ) ):
    function accesspress_mag_home_author() {
        global $post;
        $post_id = $post->ID;
        $apmag_author_option = of_get_option( 'home_author_option', '1' );
        $apmag_author_type = of_get_option( 'home_author_type', 'plain_text' );
        switch ( $apmag_author_type ) {
            case 'box_text':
                $author_style = 'boxed-style';
                break;
            
            default:
                $author_style = 'plain-style';
                break;
        }
        if( $apmag_author_option == '1' ) {
    ?>
        <span class="home-post-author <?php echo $author_style; ?>"><?php the_author_posts_link(); ?></span>
    <?php
        }
    }
endif;
 ...
 */
if( !function_exists( 'accesspress_mag_block_title' ) ):
    function accesspress_mag_block_title( $block_cat ) {
        $cat_link_option = of_get_option( 'home_cat_link', '' );
        $category_info = get_category_by_slug( $block_cat );
        $category_id = $category_info->cat_ID;
        $category_link = get_category_link( $category_id );
        $category_name = $category_info->name;
?>
    <h2 class="block-cat-name">
        <span>
            <?php 
